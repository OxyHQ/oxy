...
function filler(extra = {}) {
  return {
    ...
    "packages/api/src/__tests__/healthGate.test.ts":
      "// `??=` so a real value always wins. No MONGODB_URI placeholder: it is not read.\n",
    "packages/api/src/config/__tests__/env.test.ts":
      "it('boots with no MONGODB_URI at all — Mongo left the serving path', () => {\n"
      + "  delete process.env.MONGODB_URI;\n"
      + "});\n",
    "packages/create-oxy-app/src/__tests__/backendTemplate.test.ts":
    ...
    "scripts/check-deploy-secrets-sync.mjs":
      ...
      + "// still be in the required array. MONGODB_URI left the serving path on 2026-08-08.\n",
    ...extra,
  };
}
...
const cases = [
  ...
  // ------------------------------------------------------------ manifests ---
  {
    ...
    files: filler({
      "packages/backend/package.json": `${JSON.stringify(
        { name: "@oxy.so/api", dependencies: { mongoose: "^8.24.0" } },
        null,
        2,
      )}\n`,
    }),
  ...
  },
  {
    ...
    files: filler({
      "packages/backend/package.json": `${JSON.stringify(
        { name: "@oxy.so/api", devDependencies: { "mongodb-memory-server": "^10.0.0" } },
        null,
        2,
      )}\n`,
    }),
  ...
  {
    // The regression this check exists for. `@oxy.so/api` described itself
    ...
    files: filler({
      "packages/backend/package.json": `${JSON.stringify(
        { name: "@oxy.so/api", description: "Express 5 / Mongoose / Socket.io backend for Mercaria" },
        null,
        2,
      )}\n`,
    }),
  ...
  },
  {
    ...
    files: filler({
      "packages/backend/package.json": `${JSON.stringify(
        { name: "@oxy.so/api", keywords: ["api", "MongoDB"] },
        null,
        2,
      )}\n`,
    }),
  ...
  },
  {
    ...
    files: filler({
      "packages/backend/package.json": `${JSON.stringify(
        { name: "@oxy.so/api", description: "Ranked among the fastest, deployed in Mongolia" },
        null,
        2,
      )}\n`,
    }),
...

  {
    ...
    // per-file. `env.test.ts` is excused for the two lines that ASSERT Mongo is
    // gone. A real MONGODB_URI read added to that SAME file must still fail, or
    // the exception is a hole rather than an allowlist.
    ...
    files: filler({
      "packages/api/src/config/__tests__/env.test.ts":
        ...
        + "  delete process.env.MONGODB_URI;\n"
        + "});\n"
        + "const revived = process.env.MONGODB_URI ?? 'mongodb://prod.internal:27017/oxy';\n",
    }),
    expectFailure: true,
    expectOutput: "carries a mongodb:// connection string",
  },
