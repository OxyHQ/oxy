...
run_release pre-task-capacity-retry true true false 0 false 1 healthy 0 '' '' '' '' 1 '' '' 3
...
grep -F 'Running Migration with arn:aws:ecs:test:task-definition/deploy-test:2' \
  "$test_directory/pre-task-capacity-retry/output.log" >/dev/null
grep -F 'migration:node packages/api/dist/db/migrate.js --phase=pre' \
...
grep -F '{"INFERENCE_ROUTING_SCORE_MIN_VALIDITY_SECONDS":"3600","KAANA_BASE_URL":"https://kaana.ai","KAANA_EDGE_SIGNING_KEY_ID":"oxy-edge-2026-08-17","KAANA_CREDENTIAL_CONTROL_SIGNING_KEY_ID":"oxy-credential-control-2026-09","INBOX_INFERENCE_ROUTING_PROFILE_ID":"${{ vars.INBOX_INFERENCE_ROUTING_PROFILE_ID }}","INFERENCE_KAANA_EXECUTION":"disabled","OTEL_SERVICE_NAME":"oxy-api","OTEL_EXPORTER_OTLP_ENDPOINT":"http://127.0.0.1:4318","OTEL_EXPORTER_OTLP_PROTOCOL":"http/protobuf","OTEL_RESOURCE_ATTRIBUTES":"deployment.environment.name=production,service.namespace=oxy"}' \
