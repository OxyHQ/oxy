...
jq -e --arg sha "$EXPECTED_SOURCE_SHA" --arg digest "$EXPECTED_IMAGE_DIGEST" '
  ...
  and (.operation == "reconcile" or .operation == "inspect_cache" or ((.operation == "inspect_meta" or .operation == "inspect_profile") and .dryRun == true))
