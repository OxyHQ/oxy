...
| [0026](0026-first-party-services-authenticate-as-workloads.md) | — | A first-party service authenticates by proving what it IS — an attestation from the infrastructure it runs on, exchanged for the same Oxy service token — instead of holding an api key and secret a human issued; the provider sits behind one interface so no caller or verifier knows which cloud we are on, and third parties keep the credential path. |
