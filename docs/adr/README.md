...
| [0011](0011-inference-data-plane-name.md) | #972 | Kaana is the canonical inference data-plane name; its only signed origin is `https://kaana.ai`, with no compatibility hostname or former-name alias. |
...
| [0013](0013-byok-secret-custody.md) | #972 | Kaana alone stores customer provider credentials, encrypted in Kaana PostgreSQL with KMS; Oxy stores exact opaque handle/revision metadata and a same-operation recovery ledger, never credential plaintext/ciphertext or any credential-derived hint. |
...
| [0015](0015-oxy-kaana-envelope-signing.md) | #972 | Ed25519 protects the signed Oxy → Kaana boundary over a domain-separated hash of the exact body; Kaana holds public keys only, rotation is additive by key id, and the edge owns idempotency. |
...
| [0024](0024-one-oxy-account-root-holders.md) | #1302 | `auth.oxy.so` is the only web entry a person is sent to; the personal root lives in user-controlled holders (Commons, passkey-sealed web envelope on an internal holder host until `auth.oxy.so` passes the holder gate); login never unlocks the root; accounts are created with their root atomically or not at all; recovery needs only root possession; every root operation carries one versioned, payload-bound, one-use proof. Supersedes parts of the 2026-09-15 two-carriers design. |
| [0025](0025-present-requester-assertions-for-native-product-agents.md) | — | A signed-in person reaches a native product agent (Homiio → Sindi) with no consent screen: the product backend trades the person's live Oxy session for a one-use, 120-second, agent- and presenter-bound `OXY-REQUESTER+JWT` that Alia verifies against the JWKS and consumes through live introspection; the human bearer never reaches Alia and `acting-as:offline` stays the lane for absent-user work. Amended 2026-09-20: an entry point may also declare the IAM role that proves it, so a product on this lane can authenticate as a workload (ADR 0026) and give up its key pair; the handle is derived from the declared role, never hard-coded. |
| [0026](0026-first-party-services-authenticate-as-workloads.md) | — | A first-party service authenticates by proving what it IS — an attestation from the infrastructure it runs on, exchanged for the same Oxy service token — instead of holding an api key and secret a human issued; the provider sits behind one interface so no caller or verifier knows which cloud we are on, and third parties keep the credential path. Amended 2026-09-19: the binding row names the token's scopes, gated as a credential's are, because otherwise no service holding a privileged scope could ever give its key pair up. |

## Related

- [`../architecture/inference-responsibility-matrix.md`](../architecture/inference-responsibility-matrix.md)
  — the committed per-table/event/API ownership record governed by the inference ADRs.
