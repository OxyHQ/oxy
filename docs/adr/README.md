...
| [0011](0011-inference-data-plane-name.md) | #972 | The inference data plane's production name IS `Relay`, and all four prohibitions are lifted, so it may ship in a published package, public documentation, a public API path/header/field and a public repository or domain; the trademark and domain/package-availability criteria were never run and remain outstanding, and the term-of-art collision with atproto's `Relay` was overruled rather than absent. |
