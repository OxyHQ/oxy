...
| [0006](0006-oxy-kaana-boundary.md) | #972 | Kaana executes inference and stores Oxy ids only as immutable references; ADR 0019 separately assigns all provider-credential ciphertext, including BYOK, to Kaana PostgreSQL/KMS. |
...
| [0011](0011-inference-data-plane-name.md) | #972 | `Kaana` is the canonical inference data-plane identity, `https://kaana.ai` is its signed origin, and the former inference-service name has no compatibility aliases; genuine SMTP, federation and transport relays keep their protocol names. |
...
| [0013](0013-byok-secret-custody.md) | #972 | Historical fail-closed decision: while Oxy had no custody backend, BYOK writes refused before reading the credential. Superseded for future custody by ADR 0019. |
...
| [0019](0019-kaana-byok-custody.md) | #972 | Every provider credential, including BYOK, lives as KMS ciphertext in Kaana PostgreSQL; Oxy stores only customer-facing metadata plus an opaque handle and revision. The architecture is accepted while both implementation cuts and production gates remain pending. |
