...
Standalone Vite app at `auth.oxy.so` — the **OAuth authorize/consent IdP** for third-party "Sign in with Oxy" (login, signup, authorize, recover, social-callback). It renders the shared `@oxy.so/services` auth surfaces via RN Web.
...
- Zod schemas in `lib/schemas.ts` for API response validation (the shared `loginResultSchema` from `@oxy.so/contracts` validates the `/auth/login` + `/auth/signup` session responses committed via `handleWebSession`)
