...
- Interactive sign-in is the in-app **`OxyAccountDialog`**, rendering THE sign-in screen — `OxySignInPanel` (`packages/services/src/ui/components/signIn/`), the one auth.oxy.so renders too: the device's accounts first ("Choose an account"), then the header, "Continue with Oxy" and "Create account". On the web both open auth.oxy.so in a window over the app, from the press (`useOxy().continueOnAuth(screen)`, ADR 0029 D1) — on every domain, `*.oxy.so` included; no code decides by domain. There the page shows the full screen: from `md` the embedded QR over a photo carousel in Bloom `AuthCard`'s split card (below `md` "Continue with Oxy"), the username + Continue (a username-first passkey — the server scopes it to that account's credentials, so a hardware security key with no resident credential works too) and "or continue with" a passkey with nothing to type. On native "Continue with Oxy" leaves the delivery route to Oxy (`selectCommonsDelivery`; see "Sign in with Oxy" below), or "Get Commons" without it. There is no password option. Open it with `useOxy().openAccountDialog()` or imperative `openAccountDialog('signin')`. It never navigates the tab to `auth.oxy.so` (a blocked window falls back to it).
...
## Web accounts — username, passkey, recovery email (ADR 0029 D3)

Inventory and invariants: [`docs/identity/holders-and-recovery.md`](../identity/holders-and-recovery.md). There is no web identity carrier (no envelope, no PRF, no web phrase, no move); the 2026-09-15 two-carriers design is reversed for passkey accounts.

- **A web account has no key.** `users.public_key` is NULL; it signs in with a passkey (RP ID `oxy.so`, asserted only on `auth.oxy.so`) and gets back in through its recovery email. Commons is the official self-custody way: `POST /auth/link` makes the account self-custodied and deletes the email.
- **Sign-up is one transaction.** `auth.oxy.so/signup` (`OxyCreateAccountPanel`): username → `POST /auth/email/verify/start {purpose:'signup', email}` → the 6-digit code → `…/confirm` → a one-use ticket → `POST /webauthn/register/{options,verify}` with `username`, `email`, `emailTicket`. The verify spends the ticket in the transaction that creates the user (with the email), the credential and its `webauthn` auth method, then mints the session (`handleWebSession`) and the page continues to the request in its query.
- **Recovery is by email.** `auth.oxy.so/recover` (`OxyRecoverAccountPanel`): username or email → `start {purpose:'recovery', identifier}` (same answer whether or not the account exists; a Commons account has no email and gets nothing) → code → ticket (+ `username`) → `register/options {recoveryTicket}` (challenge bound to the account) → `register/verify {recoveryTicket}` (ticket spent with the new passkey) → session. Old passkeys stay; the person removes lost ones in Accounts.
- **Deletion needs the passkey.** `auth.oxy.so/delete-account` (`OxyDeleteAccountPanel`): typed username → `POST /users/me/delete/options` → assertion → `DELETE /users/me {confirmText, assertion}`. A Commons account signs `delete:{publicKey}:{timestamp}` with its key, in Commons. On the web, every app's "Delete account" opens that page.
- **Linking Commons.** `auth.oxy.so/link-commons` (`OxyLinkCommonsPanel`) opens `POST /identity/link` and shows its QR; Commons (`app/(auth)/link-account/`, "I have an Oxy account on the web") scans it, creates its key without registering, signs the `link_identity` proof over the QR's challenge and posts it (`/identity/link/:linkId/proof`, no bearer); both devices show `deriveIdentityLinkCode`; the passkey completes it (`/options`, `/complete`). `linkRootToAccount` (`services/identityLink.service.ts`) is the one place a root is first linked — also behind `POST /auth/link` — and it deletes the recovery email in the same transaction. Commons then signs in with its key.
- **Only `auth.oxy.so`.** `/auth/email/*` answers only `getAuthWebOrigin()` and loopback; sign-up and recovery ceremonies from any other origin are refused. Codes are HMAC'd, tickets hashed, emails hashed (`email_verifications`); rate limits by `hashedIpKey` and per hashed email. Mail: `smtpOutbound.sendSystem` from `noreply@`.
- **The recovery email is not a profile field.** `PUT /users/me` does not write `email`, `/auth/register` (Commons) ignores one, and `GET /auth/check-email` is deleted.
- An Oxy app reaches these pages in auth.oxy.so's window over the app: `useOxy().continueOnAuth(screen)` opens `/authorize?screen=signin|signup|recover` with `response_mode=web_message`, and the code it posts back signs the app in (ADR 0029 D1). The origin's `sensitive` headers carry the holder policy (ADR 0028 D2) — never COOP, which would sever the third-party OAuth popup's `window.opener`.

## Sign in with Oxy — QR/Shared-Key Handoff (PR #415, extended by issue #691)

**User-facing label everywhere: "Sign in with Oxy"** — never say "Sign in with Commons"; the mechanism is invisible plumbing. The in-app `OxyAccountDialog` entry is the shared sign-in screen (see "Interactive sign-in" above): "Continue with Oxy" leaves it to Oxy — not the user — how the request reaches the Commons identity (see "Automatic delivery selection" below), and the embedded QR is a QR-only request (`AccountDialogController.startInlineQr`, `signIn.inline`) that pushes to no phone and opens no Commons, because the screen starts it by itself. There is no password option anywhere in it.
...
- `OxyAuthChooser`'s `signin`/`add` entry is `OxySignInPanel` and `signup` is `OxySignUpPanel` (`packages/services/src/ui/components/signIn/`) — the same screens auth.oxy.so's `/login` and `/signup` render. The ACTIVE REQUEST (`qr` view, `OxySignInRequestSurface`) still keeps its alternatives behind "Having trouble?" (`authChooser/TroubleDisclosure.tsx`), auto-revealed only when the chosen route reports `signIn.routeFailed`.
...
Standalone Vite app at `auth.oxy.so` — the **OAuth authorize/consent IdP** for third-party "Sign in with Oxy", plus the MCP (`/mcp/link`) and CLI (`/device`) approval pages. It owns NO sign-in UI: `/login` and `/signup` render the SDK's own screens (`OxySignInPanel`, `OxySignUpPanel` from `@oxy.so/services`), the same ones every Oxy app's account dialog renders, via RN Web.

**ARCHITECTURE: the auth app is a device-first origin AND the OAuth authorize/consent IdP — NOT a Relying Party**
- It mounts `OxyProvider` from `@oxy.so/services` with NO special props (`packages/auth/src/main.tsx`): it runs the SAME device-first cold boot every Oxy app runs (restore THIS origin's device session from its own persisted `{deviceId, deviceSecret}`), enumerates the device directory through `useDeviceSwitcher`, and signs in through the SDK's own screens and funnels (`signInWithPasskey`, the account dialog's Commons request). There is NO transport/chooser exception — the IdP is a device-first origin like accounts.oxy.so.
