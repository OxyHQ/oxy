...
- Interactive sign-in is the in-app **`OxyAccountDialog`**, rendering THE sign-in screen — `OxySignInPanel` (`packages/services/src/ui/components/signIn/`), the one auth.oxy.so renders too: the device's accounts first ("Choose an account"), then the header, the Commons way in (on the web, from `md`, the embedded QR over a photo carousel in Bloom `AuthCard`'s split card — the dialog grows to 880 for it; below `md` and on native, "Continue with Oxy", or "Get Commons" on a native device without it), the username + Continue (a username-first passkey — the server scopes it to that account's credentials, so a hardware security key with no resident credential works too), "or continue with" a passkey with nothing to type (both run on the page on an `oxy.so` origin, on auth.oxy.so in the same tab elsewhere; none on native), and "Create account". There is no password option. "Continue with Oxy" leaves the delivery route to Oxy (`selectCommonsDelivery`; see "Sign in with Oxy" below). Open it with `useOxy().openAccountDialog()` or imperative `openAccountDialog('signin')`. It never navigates to `auth.oxy.so`.
...
## Web identity carrier — `auth.oxy.so` (one identity, two carriers)

Design: `docs/superpowers/specs/2026-09-15-one-identity-two-carriers-design.md`; the holder is `auth.oxy.so` since [ADR 0028](../adr/0028-the-identity-carrier-is-auth-oxy-so.md) (`id.oxy.so` is gone).
...
- **Oxy is never the custodian.** The API stores the sealed envelope (`identity_web_envelopes`) and can open none of it. `GET/PUT/DELETE /identity/web-envelope`, `POST …/establish` and `POST …/phrase-confirmed` answer ONLY `AUTH_WEB_ORIGIN` (`https://auth.oxy.so`, the API's `getAuthWebOrigin()`) or loopback, and every write carries a fresh identity-key proof bound to its action. A first identity is linked and stored in ONE transaction (`/establish`) — never a linked key without its envelope.
- **The holder logic is auth.oxy.so's, and only its.** `packages/auth/lib/identity/` (carrier, passkey, envelope cache, API client) runs on its OWN `OxyServices` client with an in-memory bearer, never on the origin's `OxyProvider` session, and never ships in `@oxy.so/services`. The screens (`src/pages/signup.tsx`, `recover.tsx`, `identity.tsx`, `prf-check.tsx`) are built from the SDK's sign-in shell and Bloom. The origin's `sensitive` headers carry the holder policy (ADR 0028 D2) — never COOP, which would sever the third-party OAuth popup's `window.opener`.
- **Key operations stay on that origin.** Saving/recovering the phrase and account deletion run on `auth.oxy.so/identity` with the carrier's own session; no other site receives the key or a signature made with it. Creation and recovery that ARE a sign-in (`/signup`, `/recover`) hand their session to the origin's `OxyProvider` (`handleWebSession`) and continue to the request in the query. An Oxy app reaches them — and an `oxy.so` passkey from another domain — IN THE SAME TAB: `useOxy().continueOnAuth(screen)` redirects to `/authorize?screen=signin|signup|recover` and the ordinary code return brings the person back signed in (ADR 0028 D1b). There is no identity popup.
...
- **Moving to Commons is a MOVE, never a copy.** `auth.oxy.so/identity/move` starts `POST /identity/move` (bearer, identity origin) and shows a QR carrying only the 128-bit move id (`oxycommons://move?id=`); Commons scans it from Restore → "Move from the Oxy website" (`app/(auth)/import-identity/scan.tsx` — `(scan)` needs a session) and joins without a bearer. Both screens derive the same 6-digit code from the move id and BOTH ephemeral keys (`deriveMoveSas`); the web seals the entropy only after the person confirms they match, with an identity-key proof. Commons imports it like a phrase import and posts a receipt signed by the identity key; the web verifies that receipt LOCALLY before deleting the envelope (server + IndexedDB), so the relay cannot fake a completed move. The relay row (`identity_moves`) holds two ephemeral public keys and an opaque ciphertext cleared on completion, cancel or expiry; every transition is one conditional UPDATE on status and deadline.
