...
- **The holder logic is auth.oxy.so's, and only its.** `packages/auth/lib/identity/` (carrier, passkey, envelope cache, API client) runs on its OWN `OxyServices` client with an in-memory bearer, never on the origin's `OxyProvider` session, and never ships in `@oxy.so/services`. The screens (`src/pages/continue.tsx`, `identity.tsx`, `prf-check.tsx`) are built from the SDK's sign-in shell and Bloom. The origin's `sensitive` headers carry the holder policy (ADR 0028 D2) — never COOP, which would sever the popup's `window.opener`.
- **Key operations stay on that origin.** Saving/recovering the phrase and account deletion run on `auth.oxy.so/identity` with the carrier's own session; no other site receives the key or a signature made with it. `/continue?user_code=` (never `?code=`: the origin's `OxyProvider` cold boot consumes that) signs an app in through the existing device-flow `authorize-code` path with the mandatory confirmation + acknowledgement.
