...
### Services (`@oxy.so/services`) — Trust screens
...
- `rl:auth:challenge:`, `rl:auth:verify:`, `rl:auth:refresh:`, `rl:auth:lookup:`, `rl:auth:session-claim:`, `rl:auth:oauth-authorize:`, `rl:auth:oauth-consent:`, `rl:auth:oauth-token:`, `rl:auth:login:`, `rl:auth:client-lookup:`
- `rl:auth:service-token:` (keyed on the presented `apiKey`, 30 / 5min), `rl:auth:service-token-address:` (per-address flood ceiling, 600 / 5min), `rl:auth:service-token-workload:` — the service-token mint. `isServiceTokenMintPath` exempts it from `rl:general`, `rl:auth` and `slowDown`, because every service mints from the cluster's one NAT address with no bearer yet
- `rl:inference:catalogue:` (per address, anonymous/public viewers) and `rl:inference:catalogue:service:` (per `appId:credentialId`); likewise `rl:inference:reporting:*` / `rl:inference:routing:*` with `service-*` twins — an application credential skips the address bucket for its own (`utils/serviceRateLimitKey.ts`)
...
## Federation (`@oxy.so/federation`)

`packages/federation` → `@oxy.so/federation` (published to npm — `package.json` is the version source of truth, not this file) is the app-agnostic ActivityPub identity + follow engine substrate: the network-connector contract, normalized cross-network DTOs, HTTP signatures, the local-actor builder, and remote-actor resolution. The isomorphic `.` entry stays free of Express/Mongoose so it can be imported anywhere; the runnable engine (signed fetch, delivery transport, webfinger/actor/inbox routers) lives under the separate `./node` subpath so it never enters isomorphic bundles. Mention is the connector consumer; oxy-api imports only pure functions off the isomorphic entry — host canonicalisation (`canonicalFederationHost`, `isSameFederationHost`), the upstream-URL parser (`federatedUsernameFromUpstreamUrl`), and HTTP-signature signing (`signRequest`) — never the connector contract or the `./node` Express routers.
...
- **Publishing CLI:** `@oxy.so/ship` (`packages/ship`, bin `oxy-ship`) parses an `expo export` output, hashes assets, and publishes to a channel via the admin API — see `packages/ship/README.md`.
...
**Post-PR #415: Accounts is KEYLESS and management-only.** All identity creation, key management, recovery phrase, backup, and key-based flows moved to `packages/commons`. Accounts signs in via the shared `OxyAccountDialog` from `@oxy.so/services` — email (code or link), an optional password and authenticator, or "Continue with Oxy" / the Commons QR (ADR 0030). Its security settings manage the password, the authenticator and its backup codes, and linking Commons (services panels over `routes/accountSecurity.ts` and `/identity/link`). Account deletion is the services `OxyDeleteAccountPanel` (a code emailed for the deletion, ADR 0030 D6); a Commons account deletes in Commons (`commons://delete-account`).
...
- **Removed unused deps**: `@radix-ui/react-tabs`, `react-responsive`, `@lottiefiles/dotlottie-react-native`, `expo-symbols`. KEEP `expo-document-picker` + `expo-image-manipulator` (lazy-loaded optional peers of `@oxy.so/services`) and `@lottiefiles/dotlottie-react` (hard-required by web lottie export).
...
**Never `pm clear` an Oxy app on a device holding a real identity; uninstall it instead.** `adb shell pm clear <pkg>`, and *Settings › Apps › <app> › Storage › Clear storage*, call `clearApplicationUserData`, which wipes the Android Keystore of the WHOLE `so.oxy.shared` UID — every Oxy app's expo-secure-store keys and the androidx master key, not just that package's (observed on a Pixel 8a, oxy#1388). Uninstalling one package leaves the UID's Keystore alone as long as another package still belongs to the UID. Commons recovers its identity from the Block Store device backup when its binary has it ([device-backup.md](../identity/device-backup.md)); every other app still loses its encrypted prefs and signs in again, and a Commons binary without the backup falls back to the recovery phrase.
...
**Civic contracts:** `packages/contracts/src/civic.ts` — Zod schemas + inferred types for all civic surfaces. Consumed as `workspace:*` by api/core/services. **NOT published to npm.** Do not bump `@oxy.so/contracts` version for civic-only additions until Fases 0–4 are deployed and stable.
...
- **"In memory" is not an exemption to the invariant.** `packages/protocol/src/node/rateLimit.ts` (the self-hosted / managed node's own limiter, in the PUBLISHED `@oxy.so/protocol`) keyed its in-process window `Map` on a raw `req.ip ?? 'unknown'` — never Redis, never disk, the mildest form of the problem and still the form. It now keys on `clientRateLimitKey`, an HMAC under a 256-bit salt drawn from `randomBytes` at module load and held only in memory. The salt is deliberately EPHEMERAL: a rate-limit window is short-lived and process-local, so nothing needs a key that survives a restart or means anything on another node — which makes a per-process salt both sufficient and better than a configured one (nothing to distribute, nothing to rotate, nothing to leak, and the mapping dies with the process). A node operator supplies no salt and cannot. Note the reach: `@oxy.so/protocol` is published, so this lands for consumers only on its next publish.
