...
**Date:** 2026-09-15
**Status:** Approved 2026-09-15; **superseded by [ADR 0030](../../adr/0030-email-code-password-authenticator.md) (2026-09-26): there is no passkey; an account without a key signs in by email, an optional password and an optional authenticator, and its "no password"/"no email" lines no longer hold;** **the web carrier is deleted by [ADR 0029](../../adr/0029-one-oxy-session.md) D3** (web accounts are passkey + recovery email, no web identity); earlier **partly superseded by [ADR 0024](../../adr/0024-one-oxy-account-root-holders.md)** (#1302): the identity origin is an internal holder host, not a product; moving is one of two holder operations (add or move); decision D2's session block is replaced by readiness metadata; the `{action,userId,timestamp}` proof is replaced by the v2 payload-bound proof. Read the ADR first.
