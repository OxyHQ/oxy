...
## What production is signed with
...
| **Service token** | 1 hour (`SERVICE_TOKEN_EXPIRY`) | **`SERVICE_TOKEN_PRIVATE_KEY`**, EdDSA (Ed25519) under `SERVICE_TOKEN_SIGNING_KEY_ID`, `iss: oxy-auth`, `aud: oxy-api` | `packages/api/src/services/serviceTokenMint.service.ts` |

**Service tokens no longer share the user-token key.** Per
[ADR 0012](../adr/0012-service-token-signing-key-model.md) they are EdDSA only,
with public keys at `https://api.oxy.so/.well-known/jwks.json`; HS256 service
tokens are refused everywhere since 2026-09-25 (#877). Rotating
`ACCESS_TOKEN_SECRET` therefore touches user tokens only, and the service-token
...
- **`ACCESS_TOKEN_SECRET` or `REFRESH_TOKEN_SECRET` was exposed.** The known
  ...
  a copy can mint a user access token for any account. It can no longer mint a
  service token: those are EdDSA only. Treat as the most severe credential
  incident on this list.
- **A service outside `oxy-api` was given the secret.** Nothing outside `oxy-api`
  needs it: service-token verifiers use the public JWKS. If one holds it, rotate
  and remove it from the other side.
- **Scheduled.** Nothing forces it and the cost is real.
...
## Additive Ed25519 rotation of the service-token key
...
1. Publish the new public key in the JWKS — verifiers now know both `kid`s. Nothing
   is signed with the new key yet and nothing is weaker. As of 2026-09-25
   `/oxy/oxy-api/SERVICE_TOKEN_PUBLIC_JWKS` does NOT exist and is not in
   `TASK_SECRET_OVERRIDES_JSON` (`.github/workflows/deploy-aws.yml`): create the
   parameter first, then add the binding in the same PR — a binding to a missing
   parameter fails task launch with `ResourceInitializationError`.
...
### Emergency: the service-token private key is exposed

Anyone holding `SERVICE_TOKEN_PRIVATE_KEY` can mint a service token naming any
application until the key stops being published. Tokens are verified by `kid`,
so withdrawal is what ends it:

1. Generate a fresh key and a new `kid` (`openssl genpkey -algorithm ed25519`).
2. Set `/oxy/oxy-api/SERVICE_TOKEN_PRIVATE_KEY` and
   `/oxy/oxy-api/SERVICE_TOKEN_SIGNING_KEY_ID` to the new pair, and leave the
   compromised public key OUT of `SERVICE_TOKEN_PUBLIC_JWKS` — skip the grace
   window: honest callers holding old-`kid` tokens get 401 and re-mint, which is
   the price of refusing the attacker's.
3. Deploy, then confirm `https://api.oxy.so/.well-known/jwks.json` lists only the
   new `kid`.
4. Expect a burst of 401s for up to five minutes (the verifiers' JWKS cache) plus
   one token lifetime. SDK verifiers refresh on an unknown `kid` at most once a
   minute, so new tokens verify without a restart.

**Do not roll back to HS256.** That path is removed, and restoring it would
re-create the shared-secret mint key ADR 0012 retired.
