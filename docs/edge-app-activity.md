...
Accounts (`oxy-accounts`) and Console (`oxy-console`) observe requests in Workers before the ASSETS binding. Auth (`oxy-auth`) uses root Pages directory middleware (`functions/_middleware.ts`), its only function. All assets and navigations are included; the original Response, headers, cookies, streams and errors remain owned by their existing handlers.
