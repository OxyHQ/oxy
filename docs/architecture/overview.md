...
OxyHQServices (`@oxy.so/sdk`, a private Bun-workspaces + Turbo monorepo) is the
...
2. **A client SDK** (`@oxy.so/core`, `@oxy.so/services`, `@oxy.so/contracts`,
   `@oxy.so/protocol`) that every Oxy app (Mention, Allo, Homiio, Syra, accounts,
   console, …) consumes for auth, session, profiles, payments, and media.
   `@oxy.so/services` is the **single UI SDK** — `OxyProvider` serves Expo,
...
3. **A self-sovereign identity layer** — "Oxy ID": `did:web` documents,
   ...
   **Commons** vault app.
4. **A decentralization layer** — user-operated **data nodes** (`@oxy.so/node`)
...
node, and in any third-party verifier — using the exact same `@oxy.so/core` code.
...
```
packages/
  contracts/      @oxy.so/contracts   Zod API contracts (zero React/RN/Expo; only zod)
  protocol/       @oxy.so/protocol    Signed-record envelope, canonical JSON, platform crypto
  core/           @oxy.so/core        Platform-agnostic foundation (client + SessionClient + /server)
  services/       @oxy.so/services    The single UI SDK (OxyProvider — Expo/RN/RN Web)
  api/            @oxy.so/api         Express.js backend (api.oxy.so) — PRIVATE
  node/           @oxy.so/node        Self-hostable personal data-node server — PRIVATE
...
provider and hooks were merged into `@oxy.so/services`, which web apps consume
...
> npm publish. `@oxy.so/api`, `@oxy.so/node`, and the web IdP are private and
> deploy from source.
...
`tsc`, because they import `@oxy.so/core/server`.

---

## 4. Package boundaries (strict — enforced)

- **`@oxy.so/contracts`** — never imports `react`, `react-native`, or `expo-*`.
  Only `zod`. Both server and client import contract types directly from it.
- **`@oxy.so/protocol`** — platform-agnostic crypto/records substrate; never
  imports `react`, `react-native`, or `expo-*`.
- **`@oxy.so/core`** — never imports `react`, `react-native`, or `expo-*`. Dynamic
...
- **`@oxy.so/services`** — does **not** re-export from `@oxy.so/core` or
  `@oxy.so/contracts`. Consumers import core types from `@oxy.so/core` and contract
  types from `@oxy.so/contracts` directly.
- **`@oxy.so/api`** — imports schemas from `@oxy.so/contracts`; server auth helpers
  from `@oxy.so/core/server` only. No re-export shims, no `@deprecated` aliases —
  breaking changes are clean cuts.

There is a hard platform split in `@oxy.so/core`: client code lives under
...
is published as the `@oxy.so/core/server` subpath. The server subpath declares
...
sequenceDiagram
    ...
    participant SDK as @oxy.so/core OxyServices
    participant API as api.oxy.so (@oxy.so/api)
...
- **Session authority is the server + SDK.** The app never hand-plants tokens
  ...
  (`runSessionColdBoot` in `@oxy.so/core`) restores the session silently, and
...
- **Display names are an API contract.** `name.displayName` is **optional**
  ...
  `getNormalizedUserHandle` from `@oxy.so/core` — never recompose from
  `name.first`/`name.last`/`username`.
- **Identity/handle normalization** is shared (`getNormalizedUserHandle` in
  `@oxy.so/core`); apps don't build local route helpers.
- **Backend identity** comes from `@oxy.so/core/server` (`getRequiredOxyUserId`);
...
- `@oxy.so/api` → AWS ECS Fargate (`us-west-2`, cluster `oxy-cluster`), behind an
...
- `@oxy.so/node` → self-hosted by users (Docker + Caddy) or, for the managed
