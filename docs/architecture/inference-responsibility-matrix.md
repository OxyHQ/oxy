...
| Provider / BYOK connection metadata schema (no secrets) | Oxy | `packages/contracts/src/inference/providerConnection.ts` (`providerConnectionSchema` carries only exact opaque `credentialHandle` + `credentialRevision` custody metadata — ADR 0013) | exists |
