...
| Routing controls and fallback semantics documentation | Oxy | OxyHQServices `docs/inference/routing.md` | exists — every control, and an explicit split between what is enforced (`defaultTarget`, version pinning, no silent substitution, write-time validation) and what is stored-versioned-pinned-and-never-consulted (everything else, tracked as [#1011](https://github.com/OxyHQ/oxy/issues/1011)) |
