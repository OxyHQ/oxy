...
| Price / latency / throughput / balanced ranking | Oxy (score evidence and signed order), Kaana (execute order) | OxyHQServices `packages/api/src/services/inferenceCatalogue.service.ts`, `inferenceEdge.service.ts`; Kaana `internal/kaana/executor.go` | exists in source — profile priority precedes score descending; exact deployment ID code units are only the deterministic tie-break. Missing, stale, mismatched or colliding identity/price/score evidence refuses before reservation; names and database order never select |
