...
| Item | Owner | Repo / path | Status |
|---|---|---|---|
| `availabilityScope` (`platform_internal`, `public_payg`, `enterprise`, `byok_only`, `oxy_hosted`) | Oxy | OxyHQServices `packages/api/src/db/schema/inferenceDeployments.ts:168,291-294` | exists — the column's enum is taken from the contract's own zod enum rather than restated, so the column, the CHECK and the wire cannot disagree |
...
| "Available to Oxy products internally" vs "available to external customers" separation | Oxy | OxyHQServices `packages/api/src/db/schema/inferenceDeployments.ts` (`availability_scope`), vocabulary in `packages/contracts/src/inference/catalogue.ts` | v3 contract set — `modelDeploymentSchema` v2 and `modelCatalogueEntrySchema` v3 carry `platform_internal`; selectability filters every route to the viewer's permitted scopes, and the model-level field appears only when every visible deployment agrees, never from a name-selected representative |
