...
| Provider secret material | Kaana | OxyHQ/Kaana PostgreSQL credential store + KMS | accepted architecture in ADR 0019 and implemented in draft Kaana #48: all provider credentials, including BYOK, are KMS ciphertext in Kaana PostgreSQL. Not merged or production-verified |
...
| Environment/account encryption separation | Kaana | Kaana KMS encryption context and exact resolver | draft — ciphertext binds provider, owner account, connection, environment, handle and revision; production IAM/database-role proof remains pending |
...
| Rotation, replacement, immediate disable | Oxy control; Kaana custody | Oxy provider-connection routes + Kaana signed mutation endpoint | draft for Kaana custody — disable remains immediate Oxy metadata; create/rotate/revoke use exact expected revision and uncertain outcomes remain unroutable for reconciliation |
...
| BYOK behaviour and limits documentation | Oxy | OxyHQServices `docs/inference/byok.md` | accepted architecture documented — all provider credentials are KMS ciphertext in Kaana PostgreSQL; Oxy retains metadata plus an opaque handle/revision. Kaana #48 and the coordinated Oxy cut remain draft, so the merged create/rotate path still refuses with `503` rather than pretending the custody boundary is live |
