...
- **MongoDB (optional)** — only needed by the remaining one-shot admin scripts under `packages/api/scripts/`. The API serving path does NOT connect to Mongo, no test suite needs it, and `docker-compose.dev.yml` no longer ships a service for it. If a script needs one: `mongod --dbpath /var/lib/mongodb --bind_ip 127.0.0.1 --port 27017`.
