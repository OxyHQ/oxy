...
**Pure-static SPA — NO Pages Function.** The device-account chooser is served entirely by the device-first SDK (`useDeviceSwitcher`), so `packages/auth/functions/` and its `/api/device-accounts` feed were DELETED in the 2c cutover. The IdP is now a pure-static Vite SPA that CF serves directly, with SPA history-fallback for unmatched navigations.
