/**
 * Expo Config Plugin: withOxyAndroidWebp
 *
 ...
 *    Converting them (and renaming to `.webp`) still wins against the DEFLATED
 *    png. Renaming is safe because an Android resource is addressed by its
 *    filename without the extension: `@drawable/splashscreen_logo` resolves
 *    either way.
 *
 ...
 *
 * REGISTRATION ORDER: register this FIRST in the app's `plugins` array so that
 * it RUNS LAST. `withMod`'s `interceptingMod` awaits its own action and only
 * then calls `nextMod` (the previously registered mod), so the mod chain
 * executes in reverse registration order — and this has to run after every
 * image generator (splash screens included), or a generator that rewrites a
 * `.png` after the conversion leaves the tree with both a `.png` and a `.webp`
 * claiming the same resource name.
 *
 * `sharp` is an optional peer dependency: apps that enable this plugin must
 * install it (`bun add -d sharp`).
 *
 ...
 * On a real launcher icon that accounts for every difference. Comparing the raw
 * bytes instead would reject a conversion that is genuinely lossless.
 */
...
async function convertResourceImages(projectRoot, resDir) {
  ...
  try {
    ...
    throw new Error(
      `[withOxyAndroidWebp] \`sharp\` is required to emit real WebP resources but could not be loaded (${error.message}). ` +
        'Install it as a devDependency of the app.',
    );
  }
  ...
  if (files.length === 0) {
    ...
    throw new Error(
      `[withOxyAndroidWebp] Found no bitmap resources under ${path.relative(projectRoot, resDir)}. ` +
  ...
  for (const file of files) {
    ...
    if (!source.subarray(0, 4).equals(PNG_MAGIC)) {
      throw new Error(
        `[withOxyAndroidWebp] ${path.relative(projectRoot, file)} is neither WebP nor PNG; refusing to guess its format.`,
      );
    }
    ...
    if (!rendersIdentically(before, after)) {
      throw new Error(
        `[withOxyAndroidWebp] Lossless conversion of ${path.relative(projectRoot, file)} changed a visible pixel; refusing to write it.`,
      );
    }
    ...
    if (target !== file && (await exists(target))) {
      throw new Error(
        `[withOxyAndroidWebp] Converting ${path.relative(projectRoot, file)} would collide with an existing ${path.basename(target)}.`,
      );
    }
...
module.exports = function withOxyAndroidWebp(config) {
