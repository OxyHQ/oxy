import { routingPolicyReferenceSchema, routingPolicySchema, safeParseContract } from '../index';
...
describe('routingPolicySchema', () => {
  ...
  it('names the models a cross-model fallback may use, rather than allowing any', () => {
    ...
    expect(parsed.fallback.authorizedCrossModel).toEqual(['openai/gpt-5', 'meta/llama-4-70b']);
  ...
  it('rejects an Oxy-hosted-only policy that also requires a customer credential', () => {
    expect(
      routingPolicySchema.safeParse({ ...policy, oxyHostedOnly: true, byokPreference: 'require' })
        .success,
    ).toBe(false);
    expect(
      routingPolicySchema.safeParse({ ...policy, oxyHostedOnly: true, byokPreference: 'prefer' })
        .success,
    ).toBe(true);
  });

  it('rejects two ceilings for one unit, or ceilings in mixed currencies', () => {
    const ceiling = { unit: 'output_tokens', amount: '20.00', per: 1000000, currency: 'USD' };

    expect(
      routingPolicySchema.safeParse({ ...policy, maxPricePerUnit: [ceiling, { ...ceiling }] })
        .success,
    ).toBe(false);
  ...
  it('scopes a policy to an account or to an application, not to a user', () => {
    expect(
      routingPolicySchema.safeParse({ ...policy, scope: { kind: 'account', accountId: 'acc_1' } })
        .success,
    ).toBe(true);
    expect(
      routingPolicySchema.safeParse({ ...policy, scope: { kind: 'user', userId: 'usr_1' } })
        .success,
    ).toBe(false);
  });
});
...
describe('routingPolicyReferenceSchema', () => {
  ...
  it('refuses an unversioned reference', () => {
    expect(routingPolicyReferenceSchema.safeParse({ routingPolicyId: 'rp_1' }).success).toBe(
      false,
    );
  });
});
