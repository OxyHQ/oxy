 ...
 * session's `TimeZone` on every read; a source `Date` is an absolute UTC
 * instant, so anything but `timestamptz` silently changes what the value means.
 */
 ...
 * also truncating the ORDER BY, which makes every `created_at DESC` index
 * unusable and turns every feed page into a full sort. Removing the precision at
 * the source costs nothing and fixes every keyset, present and future.
 */
...
/**
 * `updated_at` — maintained by the APPLICATION on every `db.update()`.
 * Deliberately not a trigger: a trigger is invisible in this schema, and it
 * would also fire during backfill/maintenance writes and overwrite the
 * historical value the write exists to preserve.
 *
 ...
 *
 * Written here rather than taken from the `uuid` package: this package ships
 * both a CommonJS and an ESM build, and `uuid@14`'s package.json exports only
 * an ESM condition, so a `require()`-ing consumer cannot resolve it. Vendoring
 * twenty lines of spec avoids depending on a package whose major has to be
 * re-checked against every consumer's module system before it can be bumped.
 *
 ...
 * before the insert round-trip, so a row and its children can be built in one
 * batch.
 *
 * Rows inserted by raw SQL get no id — intended: a backfill supplies the
 * original id verbatim, which is exactly how every existing foreign key
 * survives.
 */
 ...
 * `bytea` — raw bytes.
 *
 * postgres.js decodes `bytea` to a `Uint8Array`; every consumer works in
 * `Buffer`, which IS a `Uint8Array`, so writes pass straight through and reads
 * are wrapped once here rather than at each call site.
 */
 ...
 * really is a Point at SRID 4326 is asserted against real rows in schema tests
 * instead.
 */
 ...
 *
 * Every closed value set in a schema built on these helpers is `text` plus a
 * CHECK derived from the SAME tuple that types the column, so the two cannot
 * drift. The values must be SQL literals rather than bound parameters — a
 * CHECK constraint cannot carry a parameter — which is why they are rendered
 * directly; the COLUMN itself is always interpolated as a drizzle Column so its
 * SQL name still comes from the casing setting rather than being spelled out
 * here.
 *
 ...
 * handed a runtime value: a value containing a quote is interpolated verbatim,
 * not rejected.
 */
 ...
 * spelling its values out independently, where the tuple and the constraint can
 * silently stop agreeing with each other.
 *
 * Rendering is deliberately `join(', ')` so the emitted DDL is byte-identical to
 * what an already-applied migration contains — a cosmetic difference here would
 ...
 * closed value set. A CHECK written `<@` (containment) against this literal is
 * trivially satisfied by an empty array.
 *
