 ...
 *
 * Walking the `cause` chain once, here, is what keeps every "a unique index
 * rejected a concurrent duplicate" branch honest, across any backend that
 * relies on catching a duplicate rather than reading first.
 *
 * `constraintName` is what makes a reaction SPECIFIC. `isUniqueViolation(error)`
 * alone cannot tell "this particular index fired" from "some unrelated unique
 * index fired", so a handler that maps a duplicate onto a retry must name the
 * constraint it is answering for — otherwise a future index on the same table
 * quietly starts triggering the wrong recovery.
 */
