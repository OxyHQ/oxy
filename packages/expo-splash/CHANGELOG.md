...
`@oxy.so/expo-splash` is now Apache-2.0. The code, the API surface and the behaviour are
...
`@oxy.so/expo-splash` is below 1.0.0, where semver puts the breaking position in the minor
