...
Everything above is in `@oxy.so/core` — `listStoreApps`, `getStoreApp`, `listStoreReviews`, `writeStoreReview`, `getAppListing`, `writeAppListing`, and the rest. Prefer those over hand-rolled requests: they know which endpoint uses which envelope, which is the one thing about this API that is easy to get silently wrong.
