 ...
 * `__tests__/providerSecretLeak.test.ts`, issue #972 workstream 10). A mutation
 ...
 * to the socket. `providerSecretLeak.test.ts` plants a leak only one of them
 * catches, so neither is decorative.
 *
 * Three things it CANNOT see, measured in that file rather than assumed:
 *
 *  - A `#private` field. `ProviderSecretValue` holds its plaintext in one, so an
