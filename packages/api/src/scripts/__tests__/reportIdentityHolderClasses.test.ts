...
it('counts each class once, and reports nothing but numbers', async () => {
  ...
  // A mnemonic web holder with a verified wrap and a saved phrase.
  ...
  const sealedMnemonic = sealWebIdentity(mnemonic, { prfOutput: new Uint8Array(32).fill(1), credentialId: 'census-aaaaaaaaaaaaaaaaaa', rpId: 'oxy.so' });
  ...
  // A raw-key holder whose root was since rotated away (stale).
  ...
  const sealedRaw = sealWebIdentity(raw, { prfOutput: new Uint8Array(32).fill(2), credentialId: 'census-bbbbbbbbbbbbbbbbbb', rpId: 'oxy.so' });
  ...
  expect(delta((c) => c.webHolders.mnemonic)).toBe(1);
  expect(delta((c) => c.webHolders.rawKey)).toBe(1);
