...
export const users = pgTable(
  'users',
  {
     ...
     * clean cut, not a backfill: existing rows start NULL, and the account
     * holder sets it themselves through the Accounts app. Going forward this
     * is the field every write path should set; `birthday` is kept only for
     * backward compatibility with existing readers (see `user.service.ts`'s
     * `updateUserProfile`).
     *
