...
export const inferenceDeploymentRoutingScores = pgTable(
  'inference_deployment_routing_scores',
  {
    ...
    fundingClass: text({ enum: INFERENCE_FUNDING_CLASSES }).notNull().default('standard_payg'),
    fundingState: text({ enum: INFERENCE_FUNDING_STATES }).notNull().default('available'),
    fundingEvidenceRef: text().notNull().default('migration/standard-payg'),
