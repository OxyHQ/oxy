 ...
 */
export function routingPolicyHighlights(policy: RoutingPolicy): Array<RoutingPolicyHighlight> {
  const target =
    policy.defaultTarget === undefined
      ...
      : policy.defaultTarget.kind === 'model'
        ...
        : `Routing profile: ${policy.defaultTarget.routingProfile}`;
  ...
  return [
    ...
    { label: 'Providers allowed', value: listOrNone(policy.providerAllowlist, 'Any') },
    { label: 'Providers denied', value: listOrNone(policy.providerDenylist, 'None') },
    { label: 'Regions allowed', value: listOrNone(policy.allowedRegions, 'Any') },
    { label: 'Regions denied', value: listOrNone(policy.deniedRegions, 'None') },
    {
    ...
    { label: 'Licences allowed', value: listOrNone(policy.allowedLicenseIds, 'Any') },
    {
...
export function catalogueLicences(
  entries: ReadonlyArray<{ license: { licenseId: string; displayName: string } }>
...
export function catalogueModelReferences(
  entries: ReadonlyArray<{ modelId: string; availableRevisions: ReadonlyArray<string> }>
): Array<string> {
