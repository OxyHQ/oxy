...
/**
 * drizzle-kit configuration for {{APP_NAME}}.
 *
 ...
 *   uses drizzle-orm's own migrator — never `drizzle-kit migrate`. drizzle-kit is
 *   a devDependency and the runtime image installs production dependencies only,
 *   so the CLI cannot reach production. See that file's docblock.
 *
 * `casing` decides what the DDL CREATES; the same constant passed to
 * `createDatabase()` in `src/db/postgres.ts` decides what queries REFERENCE. Both
 * read `DATABASE_CASING` from `@oxyhq/db`, so there is one setting rather than
 * two copies to keep in lockstep — disagreement means queries reference columns
 * the migrations never created.
 *
 * ## After every `db:generate`, add a deploy-phase marker
 *
 * drizzle-kit cannot add it. Open the new `drizzle/<tag>.sql` and add exactly one
 * line:
 *
