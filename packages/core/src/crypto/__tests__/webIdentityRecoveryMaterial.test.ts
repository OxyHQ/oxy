/**
 * Envelopes (ADR 0024 D2, D5): one root, whatever material it came from — a 12-
 * or 24-word phrase, or a raw key that must never gain a phrase — and wraps bound
 * to the RP ID their passkey lives under.
 */
...
describe('envelopes', () => {
  it.each([
    ...
    const identity = make();
    const { envelope, dataKey } = sealWebIdentity(identity, { prfOutput: prf(7), credentialId: CREDENTIAL, rpId: 'oxy.so' });
    wipeBytes(dataKey);
  ...
  it('binds the RP ID into the wrap, so a relabelled wrap does not open', () => {
    const { envelope, dataKey } = sealWebIdentity(deriveIdentityFromMnemonic(MNEMONIC_12), { prfOutput: prf(3), credentialId: CREDENTIAL, rpId: 'oxy.so' });
    wipeBytes(dataKey);
  ...
  it('refuses a secret kind that was swapped after sealing', () => {
    const { envelope, dataKey } = sealWebIdentity(deriveIdentityFromMnemonic(MNEMONIC_24), { prfOutput: prf(3), credentialId: CREDENTIAL });
  ...
  it('adds a second passkey and records which wraps have proven they open the root', () => {
    ...
    const { envelope, dataKey } = sealWebIdentity(identity, { prfOutput: prf(1), credentialId: CREDENTIAL, rpId: 'oxy.so' });
