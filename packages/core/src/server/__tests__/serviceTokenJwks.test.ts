...
import { createOxyAuthMiddleware } from '../auth';
import {
  createSigningKey,
  mockJwksFetch,
...
async function authenticate(oxy: OxyServer, bearer: string) {
  const request = {
    method: 'POST',
  ...
  const next = jest.fn();
  await oxy.middleware.auth()(request as never, response as never, next as never);
  return { request, response, next };
}

async function authenticateThroughExpress(oxy: OxyServer, bearer: string, delegatedUserId: string) {
  const request = {
    method: 'POST',
    path: '/v1/chat/completions',
    query: {},
...
describe('service-token key lifecycle against the published JWKS', () => {
  ...
  beforeEach(() => {
    ...
    oxy = new OxyServer({ baseURL: 'https://api.oxy.test' });
  });
