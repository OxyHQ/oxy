 ...
 *    (mirroring how `serviceRequest<T[]>` returns the inner array);
...
describe('OxyServer.assets.metadataByIds', () => {
  let oxy: OxyServer;
  ...
  it('returns [] and performs no network call for empty / whitespace input', async () => {
    await expect(oxy.assets.metadataByIds([])).resolves.toEqual([]);
    await expect(oxy.assets.metadataByIds(['', '   '])).resolves.toEqual([]);
  ...
  it('passes an entry\'s hlsReadyAt through untouched', async () => {
    ...
    const [entry] = await oxy.assets.metadataByIds(['asset-1']);
  ...
  it('leaves hlsReadyAt absent when the response omits it', async () => {
    serviceRequestSpy.mockResolvedValueOnce([{ ...sampleEntry, mime: 'video/mp4' }]);
  ...
  it('de-duplicates and sends a single chunk for <= 100 unique ids', async () => {
    ...
    const result = await oxy.assets.metadataByIds([
      'asset-1',
      'asset-1', // duplicate
      '   ', // dropped
    ]);
  ...
  it('chunks at 100 ids per request and merges each chunk', async () => {
    ...
    const result = await oxy.assets.metadataByIds(ids);
  ...
  it('THROWS when a chunk fails, so a failed request is never read as absence', async () => {
    ...
    await expect(oxy.assets.metadataByIds(ids)).rejects.toThrow(ServiceAssetMetadataError);
  ...
  it('returns the resolved entries when the caller opts into { partial: true }', async () => {
    ...
    const result = await oxy.assets.metadataByIds(ids, { partial: true });
