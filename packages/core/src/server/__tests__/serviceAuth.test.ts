 ...
 *
 * The tests use a real OxyServer instance with `makeRequest` stubbed and
...
import { ServiceCredentialMismatchError } from '../OxyServer';
import {
  b64url,
  createSigningKey,
  mockJwksFetch,
...
describe('C3: service-token acting-as enforcement', () => {
  ...
  it('caches positive grants per (appId, userId) — avoids hammering verify endpoint', async () => {
    ...
    // The cache logic lives in verifyActingAs itself, which we have
...
describe('H1: serviceToken per-credential cache + secret verification', () => {
  let oxy: OxyServer;
  ...
  beforeEach(() => {
    oxy = new OxyServer({ baseURL: 'http://test.invalid' });
    makeRequestSpy = jest.spyOn(oxy as unknown as { request: jest.Mock }, 'request');
  });
  ...
  it('throws even when the wrong secret has different length (no length-based bypass)', async () => {
    ...
    await oxy.serviceToken('key-A', 'secret-A-which-is-quite-long');

    await expect(oxy.serviceToken('key-A', 'short')).rejects.toBeInstanceOf(
  ...
  it('does not poison the cache when initial token fetch fails for an apiKey', async () => {
    ...
    await expect(oxy.serviceToken('key-A', 'attacker-secret')).rejects.toThrow(
...
// serviceToken() mints anew, enabling recovery from a mid-run 401 (e.g.
...
describe('H2: malformed service tokens return 401 (not 500)', () => {
  ...
  beforeEach(() => {
    oxy = new OxyServer({ baseURL: 'http://test.invalid' });
    // Ensure verifyActingAs is never reached for these tests.
    jest.spyOn(oxy, 'verifyActingAs').mockResolvedValue(null);
  });
  ...
  it('does not call onError with a 500 for any malformed-token shape', async () => {
    ...
    for (const malformed of [
      ...
      const res = makeRes();
      const next = jest.fn();
      const mw = oxy.middleware.auth({ onError });
      await mw(req as unknown as never, res as unknown as never, next as unknown as never);
    }
...
describe('H4: aud / iss / type claim verification', () => {
  ...
  beforeEach(() => {
    oxy = new OxyServer({ baseURL: 'http://test.invalid' });
    jest.spyOn(oxy, 'verifyActingAs').mockResolvedValue({ authorized: true, scopes: [] });
  });
...
describe('retired HS256 service tokens are refused', () => {
  ...
  it.each([
    ...
    for (const middleware of [oxy.middleware.auth(), oxy.middleware.service()]) {
      const req = makeReq({ headers: { authorization: `Bearer ${token}` } });
      const res = makeRes();
      const next = jest.fn();
