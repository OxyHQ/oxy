...
describe('pre-session public endpoints use skipAuth', () => {
  ...
  it('claimSessionByToken skips auth preflight', async () => {
    ...
    await oxy.auth.claimSession('secret-session-token');
  ...
  it('signUp skips auth preflight', async () => {
    ...
    await oxy.auth.signUp({ username: 'alice', email: 'a@b.c', emailTicket: 't', device: null });
    expect(makeRequest).toHaveBeenCalledWith(
      'POST',
      '/auth/signup',
  ...
  it('exchangeOAuthCode accepts a device-less grant and still plants the token', async () => {
    ...
    const result = await oxy.auth.oauth.exchangeCode({
      code: 'code-1',
      clientId: 'oxy_dk_test',
      redirectUri: 'https://app.example/callback',
      codeVerifier: 'verifier',
    ...
    expect(oxy.session.accessToken).toBe('tok');
  });
  ...
  it('getOAuthUserInfo reads the flat OIDC userinfo response', async () => {
    ...
    const result = await oxy.auth.oauth.userInfo();
