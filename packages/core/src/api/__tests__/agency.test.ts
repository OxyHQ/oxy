...
describe('native agency authority', () => {
  ...
  beforeEach(() => {
    ...
    oxy.session.setAccessToken(makeJwt({ userId: 'owner' }));
  });
  ...
  it('loads account-scoped grants and audit events', async () => {
    ...
    await expect(oxy.agency.grants.list('owner/account')).resolves.toEqual([]);
    await expect(oxy.agency.auditEvents('owner/account')).resolves.toEqual([]);
  ...
  it('re-reads the exact grant list after revocation', async () => {
    fetchMock.mockImplementation(async () => jsonResponse({ grants: [] }));
