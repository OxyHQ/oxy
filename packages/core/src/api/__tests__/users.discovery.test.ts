 ...
 *
 * `users.resolveHandle` and `users.recommendations` are best-effort discovery
 ...
 *
 *  - `users.resolveHandle` STILL returns `null` on any failure (contract unchanged),
 *    but emits a `debug` log that flags whether it was a not-found vs a failure.
 *  - `users.recommendations` STILL rethrows (contract unchanged), but emits
...
describe('discovery error handling', () => {
  ...
  describe('users.resolveHandle', () => {
    it('returns the normalized user when discovery succeeds', async () => {
      ...
      const result = await oxy.users.resolveHandle('remote@mastodon.social');
    ...
    it('returns null AND logs a not-found debug entry on 404', async () => {
      ...
      const result = await oxy.users.resolveHandle('ghost@nowhere.example');
    ...
    it('returns null AND logs a failure (not not-found) debug entry on 5xx', async () => {
      ...
      const result = await oxy.users.resolveHandle('remote@down.example');
      expect(result).toBeNull();
      expect(debugSpy).toHaveBeenCalledTimes(1);
      const [message, context] = debugSpy.mock.calls[0];
    ...
    it('returns null on a network failure (no status)', async () => {
      ...
      const result = await oxy.users.resolveHandle('remote@offline.example');
      expect(result).toBeNull();
      expect(debugSpy).toHaveBeenCalledTimes(1);
  ...
  describe('users.recommendations', () => {
    it('returns the list on success without logging', async () => {
      ...
      const result = await oxy.users.recommendations();
    ...
    it('rethrows on failure AND logs a debug entry first (contract unchanged)', async () => {
      ...
      await expect(oxy.users.recommendations()).rejects.toThrow();
      expect(debugSpy).toHaveBeenCalledTimes(1);
      const [message, context] = debugSpy.mock.calls[0];
   ...
   */
  describe('users.recommendations routing (GET vs POST)', () => {
    ...
    it('uses GET (no body) when called with no options', async () => {
      ...
      await oxy.users.recommendations();
    ...
    it('uses GET with excludeTypes as a comma-joined query param (legacy shape)', async () => {
      fetchMock.mockResolvedValueOnce(jsonResponse([{ id: 'r1' }]));
      await oxy.users.recommendations({ excludeTypes: ['federated', 'agent'] });
      const { url, init } = lastCall();
      expect(init?.method).toBe('GET');
      expect(init?.body).toBeUndefined();
    ...
    it('uses GET with limit as a query param', async () => {
      fetchMock.mockResolvedValueOnce(jsonResponse([{ id: 'r1' }]));
      await oxy.users.recommendations({ limit: 5 });
      const { url, init } = lastCall();
      expect(init?.method).toBe('GET');
    ...
    it('uses POST with the validated body when clientId is present', async () => {
      fetchMock.mockResolvedValueOnce(jsonResponse([{ id: 'r1' }]));
      await oxy.users.recommendations({ clientId: 'app-123', limit: 10 });
      const { url, init } = lastCall();
    ...
    it('uses POST when excludeIds are present', async () => {
      fetchMock.mockResolvedValueOnce(jsonResponse([{ id: 'r1' }]));
      await oxy.users.recommendations({ excludeIds: ['x1', 'x2'] });
      const { init } = lastCall();
      expect(init?.method).toBe('POST');
    ...
    it('uses POST when signalWeights are present', async () => {
      fetchMock.mockResolvedValueOnce(jsonResponse([{ id: 'r1' }]));
      await oxy.users.recommendations({ signalWeights: { graph: 3 } });
      const { init } = lastCall();
      expect(init?.method).toBe('POST');
    ...
    it('uses POST when offset is present', async () => {
      fetchMock.mockResolvedValueOnce(jsonResponse([{ id: 'r1' }]));
      await oxy.users.recommendations({ offset: 20, limit: 10 });
      const { init } = lastCall();
      expect(init?.method).toBe('POST');
    ...
    it('validates the POST body against the contract and rejects bad input', async () => {
      ...
      await expect(
        oxy.users.recommendations({ boosts: [{ userIds: ['u1'], weight: 99 }] }),
      ).rejects.toThrow();
    ...
    it('returns the scored items (score/matchedSignals) from the POST path', async () => {
      ...
      const result = await oxy.users.recommendations({ clientId: 'app-1' });
      expect(result).toHaveLength(1);
