...
describe('linking Commons', () => {
  ...
  it('refuses to sign without an identity on this device', async () => {
    ...
    await expect(oxy.identity.links.sign(LINK_ID, CHALLENGE)).rejects.toThrow('No identity');
  ...
  it('the web opens, completes with an email code and cancels', async () => {
    ...
    await expect(oxy.identity.links.create()).resolves.toMatchObject({ linkId: LINK_ID });
    await oxy.identity.links.complete(LINK_ID, reauth);
    await oxy.identity.links.cancel(LINK_ID);
