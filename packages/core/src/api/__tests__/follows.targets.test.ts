...
describe('OxyServices.followGraph', () => {
  ...
  describe('the identities the client must not be able to state', () => {
    it('never sends a follower id', async () => {
      await oxy.follows.followTarget('target-1');
      await oxy.follows.unfollowTarget('rel-1');
      await oxy.follows.list();
    ...
    it('sends no application id unless the caller explicitly names another app', async () => {
      await oxy.follows.setApplicationMode('rel-1', 'disabled');
    ...
    it('passes an explicitly named application through, for the privileged path', async () => {
      await oxy.follows.setApplicationMode('rel-1', 'enabled', 'app-9');
      expect(makeRequest.mock.calls[0][2]).toEqual({ mode: 'enabled', applicationId: 'app-9' });

      await oxy.follows.restoreInheritance('rel-1', 'app-9');
  ...
  describe('request shapes', () => {
    ...
    it('carries expiresIn for a timed follow', async () => {
      await oxy.follows.followTarget('target-1', { expiresIn: 72 * 60 * 60 });
    ...
    it('restores inheritance with no query string when the app means itself', async () => {
      await oxy.follows.restoreInheritance('rel-1');
      expect(makeRequest.mock.calls[0][1]).toBe('/v2/follows/rel-1/context');
    });

    it('paginates by cursor and filters by kind', async () => {
      await oxy.follows.list({ kind: 'oxy.topic', cursor: '2026-01-01T00:00:00.000Z', limit: 20 });
    ...
    it('releases a namespace with DELETE and encodes the segment', async () => {
      await oxy.follows.releaseNamespace('mention.dev');
      expect(makeRequest.mock.calls[0].slice(0, 2)).toEqual([
        'DELETE',
  ...
  describe('caching', () => {
    it('caches nothing', async () => {
      await oxy.follows.followTarget('t');
      await oxy.follows.targetStatus('t');
      await oxy.follows.list();
      await oxy.follows.unfollowTarget('r');
      await oxy.follows.setApplicationMode('r', 'disabled');
      await oxy.follows.restoreInheritance('r');
      await oxy.follows.claimNamespace('ns');
      await oxy.follows.releaseNamespace('ns');
