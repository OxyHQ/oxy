...
import { OXY_ACCOUNT_DELETED_EVENT_URI, OxyAccountEventError } from '../OxyServices.utility';
...
describe('verifyAccountEvent', () => {
  ...
  const client = () => new OxyServices({ baseURL: 'https://api.oxy.test' });

  it('returns the event from a genuine Oxy-signed token, verified against the public JWKS', async () => {
    const event = await client().verifyAccountEvent(eventToken(oxyKey.privateKey), { audience: 'app-mention' });
  ...
  it('reads a token without `username` (issued before the field existed) as username null', async () => {
    ...
    const event = await client().verifyAccountEvent(token, { audience: 'app-mention' });
  ...
  it('carries `retained` through for an archived account', async () => {
    ...
    await expect(client().verifyAccountEvent(token, { audience: 'app-mention' }))
  ...
  it('defaults the audience to the appId of the configured service credential', async () => {
    ...
    jest.spyOn(oxy, 'getServiceToken').mockResolvedValue(serviceToken);

    await expect(oxy.verifyAccountEvent(eventToken(oxyKey.privateKey))).resolves.toMatchObject({
      applicationId: 'app-mention',
    });
    await expect(oxy.verifyAccountEvent(eventToken(oxyKey.privateKey, { payload: { aud: 'app-other' } })))
  ...
  it.each(refusals)('refuses %s', async (_label, build, message) => {
    const attempt = client().verifyAccountEvent(build(), { audience: 'app-mention' });
    await expect(attempt).rejects.toBeInstanceOf(OxyAccountEventError);
    await expect(client().verifyAccountEvent(build(), { audience: 'app-mention' })).rejects.toThrow(message);
  });
  ...
  it('refuses a tampered payload', async () => {
    ...
    await expect(client().verifyAccountEvent(`${header}.${forged}.${signature}`, { audience: 'app-mention' }))
      .rejects.toThrow('signature is invalid');
  });

  it('refuses garbage without fetching keys', async () => {
    await expect(client().verifyAccountEvent('not-a-token', { audience: 'app-mention' }))
...
describe('listAccountEvents', () => {
  ...
  it('reads the pull feed with the service token and passes the cursor', async () => {
    ...
    jest.spyOn(oxy, 'getServiceToken').mockResolvedValue('service-token');
    const page = { events: [], nextCursor: 'cursor-1' };
    const request = jest.spyOn(oxy, 'makeRequest').mockResolvedValue(page as never);

    await expect(oxy.listAccountEvents({ after: 'cursor-1', limit: 50 })).resolves.toEqual(page);
