...
import type { OxyAuthRefusal } from '../OxyServices.utility';
import { createOptionalOxyAuth, createOxyAuthMiddleware, getOxyAuthRefusal } from '../../server/auth';
...
describe('optional auth records why a presented credential was refused', () => {
  ...
  it('records a session the API refuses to validate, without logging the session id', async () => {
    const oxy = new OxyServices({ baseURL: 'https://api.oxy.test' });
    jest.spyOn(oxy, 'validateSession').mockResolvedValue({ valid: false });
  ...
  it('keeps a validation transport failure diagnostic but id-free', async () => {
    ...
    jest.spyOn(oxy, 'validateSession').mockRejectedValue(transport);
...
describe('a blocking mount answers exactly what it always answered', () => {
  ...
  it('reports a non-optional refusal as non-optional', async () => {
    ...
    await oxy.auth()(request as never, response as never, jest.fn() as never);
