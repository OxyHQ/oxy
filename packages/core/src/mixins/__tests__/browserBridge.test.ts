...
describe('OxyServices — browser bridge', () => {
  ...
  it('registers, asks for a join code and joins without a bearer', async () => {
    ...
    expect(await oxy.registerBrowserDevice()).toEqual({ deviceId: 'dev-1', deviceSecret: 'auth-secret' });
    ...
    expect(await oxy.requestDeviceJoinCode(joinCodeRequest)).toEqual({ code: 'code-1', expiresIn: 60 });
    ...
    expect(await oxy.joinBrowserDevice(joinRequest)).toEqual({ deviceId: 'dev-1', deviceSecret: 'app-secret' });
  ...
  it('refuses a malformed bridge response and surfaces a rejected secret', async () => {
    ...
    await expect(oxy.registerBrowserDevice()).rejects.toThrow('unexpected response shape');
    makeRequest.mockRejectedValueOnce(Object.assign(new Error('invalid_device_secret'), { status: 401 }));
    await expect(
      oxy.requestDeviceJoinCode({
        deviceId: 'dev-1',
        deviceSecret: 'stale',
        clientId: 'c',
  ...
  describe('device proof on sign-ins', () => {
    ...
    it('attaches the held credential to a claim, a password sign-in and a sign-up', async () => {
      ...
      makeRequest.mockResolvedValueOnce(LOGIN);
      await oxy.claimSessionByToken('st-1', { plantTokens: false });
    ...
    it('an explicit null opts out, and a failing provider is no proof', async () => {
      oxy.setDeviceCredentialProvider(() => ({ deviceId: 'dev-1', deviceSecret: 'app-secret' }));
      makeRequest.mockResolvedValueOnce(LOGIN);
      await oxy.claimSessionByToken('st-1', { plantTokens: false, device: null });
      expect(makeRequest.mock.calls[0][2]).toEqual({ sessionToken: 'st-1' });

      oxy.setDeviceCredentialProvider(async () => {
        throw new Error('storage unavailable');
      });
      expect(await oxy.readDeviceProof()).toBeNull();
      oxy.setDeviceCredentialProvider(() => ({ deviceId: '', deviceSecret: 'x' }));
      expect(await oxy.readDeviceProof()).toBeNull();
    });
  });
});
